...
ArrayGroup Map::get_flow(std::pair<float, float> center, const Eigen::MatrixXf& occupancy, int steps, float dt)
{
    ...
    score_block.block(block_start_y, block_start_x, block_len_y, block_len_x) =
        score.block(min_y, min_x, block_len_y, block_len_x);
    ...
    motion_block.block(block_start_y, block_start_x, max_y - min_y, max_x - min_x) =
        motion.block(min_y, min_x, max_y - min_y, max_x - min_x);
    VelocityGroup velocity_group = {std::make_pair(motion_block, zero_block), std::make_pair(zero_block, motion_block),
